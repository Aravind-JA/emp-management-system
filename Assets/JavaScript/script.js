...
function displayMore(event, empID) {
    ...
    userOptions.innerHTML = `
        <a href="/employee.html?empID=${empID}">
...
const edit_emp = document.getElementById('edit_emp');
    ...
    try {
        ...
        for (var i = (currentPage - 1) * maxCountOnPage; i < Math.min(empData.length, currentPage * maxCountOnPage); i++) {
            ...
            temp += `<tr>
                <td>#${i + 1}</td>
                <td class="ver_center">
                    <img src="${imageUrl}" alt="" id="table-image">
                ...
                <td>${emp.country}</td>
                <td>
                    <button onclick="displayMore(event,'${emp.id}')">
...
function openEditEmployee(empID) {
    ...
    fetch(`http://localhost:3000/employees/${empID}`)
        ...
        .then((value) => {
            ...
            document.querySelector('#edit_emp [name="dob"]').value = newDateString;
...
async function editEmployee(empID) {
    try {
        const editEmpForm = document.getElementById('edit_user_form');
        const editEmpData = new FormData(editEmpForm);

        const dateString = document.querySelector('#edit_emp input[name="dob"]').value;
        const [year, month, day] = dateString.split("-");
        const newDateString = `${day}-${month}-${year}`;

        const selectedGender = document.querySelector('#edit_emp input[name="gender"]:checked').value;

        const userName = document.querySelector('#edit_emp input[name="firstName"]').value;

        editEmpData.set('dob', newDateString);
        editEmpData.set('gender', selectedGender);
        editEmpData.append('username', userName);
        editEmpData.append('password', newDateString);
...
function formValidation() {
    ...
    var salutationErr = firstNameErr = lastNameErr = emailErr = phoneErr = dobErr = genderErr = qualificationErr = addressErr = countryErr = stateErr = cityErr = pinErr = true;
    ...
    if ((salutationErr || firstNameErr || lastNameErr || emailErr || phoneErr || dobErr || genderErr || qualificationErr || addressErr || countryErr || stateErr || cityErr || pinErr) == true) {
        return false;
    } else {
        return true;
    }
...
function updateImage() {
    const updateUserImage = document.getElementById('update-user-image');
    // const imageUpdateBtn = document.getElementById('image-update');
